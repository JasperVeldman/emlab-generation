...
public interface PowerPlantRepository extends GraphRepository<PowerPlant> {
    ...
    @Query(value = "g.v(market).out('ZONE').in('REGION').in('LOCATION').filter{it.__type__=='emlab.gen.domain.technology.PowerPlant'}.filter{((it.constructionStartTime + it.actualPermittime + it.actualLeadtime) <= tick) && (it.dismantleTime > tick)}.out('TECHNOLOGY').filter{it.intermittent != false}.peakSegmentDependentAvailability.mean()", type = QueryType.Gremlin)
    public double calculateAveragePeakSegmentDependentAvailabilityOfIntermittentOperationalPowerPlantsInMarket(
            @Param("market") ElectricitySpotMarket market, @Param("tick") long tick);
